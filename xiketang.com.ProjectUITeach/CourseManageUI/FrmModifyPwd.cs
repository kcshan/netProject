...
namespace CourseManageUI
{
    public partial class FrmModifyPwd : Form
    {
        ...
        {
            // 数据验证（请自行完成）非空的验证, 其次，密码长度的验证。
            if (this.txtOldPwd.Text.Trim().Length < 6 || this.txtOldPwd.Text.Trim().Length > 18)
            {
                MessageBox.Show("原密码长度不正确!", "修改提示");
            ...
            {
                MessageBox.Show("俩次输入的信息密码不一致!", "修改提示");
                return;
            }
            // 【2】封装对象
